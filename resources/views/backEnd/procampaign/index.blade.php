                                ...
                                @foreach ($show_data as $key => $value)
                                    <tr>
                                        ...
                                        @if ($value->front_view == 1)
                                            <td><strong
                                                    style="background: black;color: white; width: auto; padding:8px;border-radius: 4px;">{{ $value->name }}</strong>
                                            </td>
                                        @else
                                            <td>{{ $value->name }}</td>
                                        @endif
                                        ...
                                        <td>
                                            <div class="button-list">
                                                @if ($value->status == 1)
                                                    <form method="post" action="{{ route('proCampaign.inactive') }}"
                                                        class="d-inline">
                                                        @csrf
                                                ...
                                                @else
                                                    <form method="post" action="{{ route('proCampaign.active') }}"
                                                        class="d-inline">
                                                        @csrf
                                                        <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                        <button type="button"
                                                ...
                                                <a href="{{ route('proCampaign.edit', $value->id) }}"
                                                    class="btn btn-xs btn-primary waves-effect waves-light"><i
                                                        class="fe-edit-1"></i></a>

                                                <form method="post" action="{{ route('proCampaign.destroy') }}"
                                                    class="d-inline">
                                                    @csrf
                                                    <input type="hidden" value="{{ $value->id }}" name="hidden_id">
                                                    <button type="submit"
